<div class="w-full max-w-4xl mx-auto">
    ...
    <div class="mb-8">
        ...
        <p class="text-gray-600">Plan your next adventure with our smart travel planner</p>
    </div>
    ...
    <div class="mb-8">
        <div class="flex items-center justify-between">
            ...
            <div class="flex items-center flex-1">
                <div class="flex items-center justify-center w-10 h-10 rounded-full {{ $currentStep >= 1 ? 'bg-indigo-600 text-white' : 'bg-gray-200 text-gray-600' }} font-semibold">
                    @if($currentStep > 1)
                ...
                <div class="ml-3 hidden sm:block">
                    <p class="text-sm font-medium {{ $currentStep >= 1 ? 'text-indigo-600' : 'text-gray-500' }}">Destination</p>
                </div>
            </div>
            ...
            <!-- Step 2: Dates -->
            <div class="flex items-center flex-1">
                <div class="flex items-center justify-center w-10 h-10 rounded-full {{ $currentStep >= 2 ? 'bg-indigo-600 text-white' : 'bg-gray-200 text-gray-600' }} font-semibold">
                    @if($currentStep > 2)
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                        </svg>
                ...
                <div class="ml-3 hidden sm:block">
                    <p class="text-sm font-medium {{ $currentStep >= 2 ? 'text-indigo-600' : 'text-gray-500' }}">Dates</p>
                </div>
            </div>
            ...
            <!-- Step 3: Details -->
            <div class="flex items-center flex-1">
                <div class="flex items-center justify-center w-10 h-10 rounded-full {{ $currentStep >= 3 ? 'bg-indigo-600 text-white' : 'bg-gray-200 text-gray-600' }} font-semibold">
                    @if($currentStep > 3)
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                        </svg>
                ...
                <div class="ml-3 hidden sm:block">
                    <p class="text-sm font-medium {{ $currentStep >= 3 ? 'text-indigo-600' : 'text-gray-500' }}">Details</p>
                </div>
            </div>
            ...
            <!-- Step 4: Confirm -->
            <div class="flex items-center flex-1">
                <div class="flex items-center justify-center w-10 h-10 rounded-full {{ $currentStep >= 4 ? 'bg-indigo-600 text-white' : 'bg-gray-200 text-gray-600' }} font-semibold">
                    4
                </div>
                <div class="ml-3 hidden sm:block">
                    <p class="text-sm font-medium {{ $currentStep >= 4 ? 'text-indigo-600' : 'text-gray-500' }}">Confirm</p>
                </div>
            </div>
        </div>
    </div>
            ...
            <div class="space-y-6">
                ...
                <div class="relative">
                    <input 
                        ...
                        wire:keydown.enter="searchDestinations"
                ...
                @if(count($searchResults) > 0)
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 max-h-96 overflow-y-auto">
                        @foreach($searchResults as $country)
                            <div 
                                wire:click="selectDestination('{{ $country['code'] }}')"
                        ...
                        @if($showWeatherPreview && !empty($weatherPreview))
                            <div class="mt-4 pt-4 border-t border-indigo-200">
                                ...
                                <div class="flex items-center space-x-4">
                                    ...
                                    @endif
                                    <div>
                                        <p class="text-2xl font-bold text-gray-900">{{ round($weatherPreview['temperature']) }}°C</p>
                ...
                @if($startDate && $endDate)
                    ...
                    <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
                        <p class="text-sm text-blue-800">
                            <span class="font-semibold">Trip Duration:</span> {{ $duration }} {{ $duration === 1 ? 'day' : 'days' }}
                        </p>
                    </div>
                @endif
                            ...
                            <div>
                                ...
                                <ul class="list-disc list-inside text-sm text-yellow-700">
                                    @foreach($conflictingTrips as $trip)
                                        <li>{{ $trip['destination'] }} ({{ date('M j', strtotime($trip['start_date'])) }} - {{ date('M j, Y', strtotime($trip['end_date'])) }})</li>
                                    @endforeach
                                </ul>
                            </div>
                        </div>
            ...
            <div class="space-y-6">
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
                        @foreach($tripTypes as $typeKey => $typeInfo)
                            ...
                            >
                                ...
                                <span class="text-sm font-medium {{ $type === $typeKey ? 'text-indigo-600' : 'text-gray-700' }}">
                                    {{ $typeInfo['label'] }}
                                </span>
                            </button>
                        @endforeach
                ...
                <div>
                    ...
                    <div class="flex items-center space-x-4">
                        <button 
                            type="button"
                            wire:click="$set('travelers', {{ max(1, $travelers - 1) }})"
                        ...
                        <input 
                            ...
                            min="1"
                            max="50"
                            class="w-20 text-center rounded-lg p-3 border border-gray-300 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                        >
                        <button 
                            type="button"
                            wire:click="$set('travelers', {{ min(50, $travelers + 1) }})"
                            class="w-10 h-10 rounded-lg border border-gray-300 hover:bg-gray-50 flex items-center justify-center"
                        >
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="flex items-start justify-between">
                        <div class="space-y-3 flex-1">
                            ...
                            <div>
                                ...
                                <p class="text-lg font-semibold text-gray-900">{{ $travelers }} {{ $travelers === 1 ? 'person' : 'people' }}</p>
                            </div>
                            @if($budget)
                                <div>
            ...
            <div class="flex items-center space-x-3">
                ...
                @else
                    <button 
                        ...
                        class="inline-flex items-center px-8 py-3 bg-green-600 text-white rounded-lg hover:bg-green-700 font-medium transition-colors"
                    >
                        <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                        </svg>
