<div class="w-full max-w-6xl mx-auto"
    ...
    <div class="card p-6 md:p-8">
        <!-- Step 1: Destination -->
        @if($currentStep === 1)
        <div wire:key="edit-step-1" class="space-y-6 fade-up">
            <div>
        ...
        <!-- Step 2: Dates -->
        @if($currentStep === 2)
        <div wire:key="edit-step-2" class="space-y-6 fade-up">
            <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label for="edit-start-date" class="eyebrow text-foreground-subtle block mb-2">Start Date</label>
                    <input id="edit-start-date" type="date" wire:model.live="startDate" wire:change="checkConflicts"
                        min="{{ date('Y-m-d') }}"
                        class="field p-3">
                    @error('startDate') <span class="text-destructive text-sm mt-1 block">{{ $message }}</span> @enderror
                </div>
                <div>
                    <label for="edit-end-date" class="eyebrow text-foreground-subtle block mb-2">End Date</label>
                    <input id="edit-end-date" type="date" wire:model.live="endDate" wire:change="checkConflicts"
            ...
            <x-ui.alert variant="error">
                ...
                <ul class="list-disc list-inside space-y-0.5 text-sm">
                    @foreach($conflictingTrips as $conflict)
                    <li wire:key="conflict-{{ $conflict['id'] }}">
                        {{ $conflict['destination'] }}
                        ({{ date('M j', strtotime($conflict['start_date'])) }} – {{ date('M j, Y', strtotime($conflict['end_date'])) }})
                    </li>
                    @endforeach
                </ul>
            </x-ui.alert>
        ...
        <!-- Step 3: Details -->
        @if($currentStep === 3)
        <div wire:key="edit-step-3" class="space-y-8 fade-up">
            <div>
            ...
            <!-- Budget -->
            <div>
                <label for="edit-budget" class="eyebrow text-foreground-subtle block mb-3">Budget <span class="normal-case font-normal tracking-normal">(optional)</span></label>
                <div class="relative max-w-xs">
                    ...
                    <input id="edit-budget" type="number" wire:model="budget" min="0" step="0.01" placeholder="0.00"
            ...
            <!-- Notes -->
            <div>
                <label for="edit-notes" class="eyebrow text-foreground-subtle block mb-3">Notes <span class="normal-case font-normal tracking-normal">(optional)</span></label>
                <textarea id="edit-notes" wire:model.live="notes" rows="4" maxlength="1000" class="field p-3"
        ...
        <!-- Step 4: Confirm -->
        @if($currentStep === 4)
        <div wire:key="edit-step-4" class="space-y-6 fade-up">
            <div>
                <h2 class="text-2xl font-extrabold tracking-tight text-foreground mb-1.5">Review your changes</h2>
        ...
        <div wire:key="nav-buttons-step-{{ $currentStep }}" class="flex items-center justify-between mt-8 pt-6 border-t border-border">
            ...
            @else
            <a href="{{ route('trips.show', $trip->id) }}" wire:navigate class="btn-ghost px-6 py-3">Cancel</a>
            @endif
            ...
            <button wire:click="updateTrip" x-on:click="saving = true" wire:loading.attr="disabled" wire:target="updateTrip" wire:key="update-btn"
                ...
                <x-ui.spinner wire:loading wire:target="updateTrip" size="md" />
                <svg wire:loading.remove wire:target="updateTrip" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
