<div x-data="{ unit: '{{ $units === 'imperial' ? 'imperial' : 'metric' }}', toggle(){ this.unit = this.unit === 'metric' ? 'imperial' : 'metric'; $wire.dispatch('weather-unit-changed', { unit: this.unit }); } }" class="max-w-5xl mx-auto">
    <div class="glass rounded-2xl p-6 mb-6 bg-gradient-to-br from-teal-500/10 via-emerald-500/10 to-cyan-500/10 border border-border shadow-card">
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-xl font-semibold text-foreground">{{ $location }}</h2>
                @if($current)
                    <p class="text-sm text-foreground-muted capitalize">{{ $current['condition'] ?? '' }}</p>
                @endif
            </div>
            <div class="flex items-center gap-3">
                <button type="button" @click="toggle()" class="px-3 py-1 rounded-full text-sm bg-surface-card/60 hover:bg-surface-card/80 text-foreground transition cursor-pointer">
        ...
        <div class="mt-4 grid grid-cols-1 sm:grid-cols-3 gap-6 items-center">
            <div class="flex items-center gap-4">
                ...
                @endif
                <div>
                    <div class="text-4xl font-bold text-foreground">{{ round($current['temperature'] ?? 0) }}<span class="text-xl align-super">°</span></div>
    ...
    @if(!empty($forecast))
    <div class="glass rounded-2xl p-6 bg-gradient-to-br from-teal-500/10 via-cyan-500/10 to-emerald-500/10 border border-border shadow-card">
        <h3 class="text-lg font-semibold text-foreground mb-4">7-Day Forecast</h3>
        <div class="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-4">
            @foreach($forecast as $index => $day)
                <div wire:key="forecast-{{ $index }}" class="group rounded-xl p-4 bg-surface-card/60 hover:bg-surface-card/80 transition shadow-sm">
                    ...
                    <div class="mt-2 text-center">
                        <div class="text-base font-semibold text-foreground">{{ round(($day['max_temp'] ?? $day['temperature'] ?? 0)) }}°</div>
    ...
    <div class="mt-6 rounded-xl p-4 border border-amber-300 dark:border-amber-700 bg-amber-50 dark:bg-amber-900/20">
        <div class="flex items-center gap-2 text-amber-800 dark:text-amber-300 font-semibold mb-2">
