        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <nav class="flex py-4" aria-label="Breadcrumb">
                <ol class="flex items-center space-x-4">
                    <li>
                        <div>
                            <a href="{{ route('dashboard') }}" class="text-foreground-subtle hover:text-foreground-muted">
                    ...
                    <li>
                        <div class="flex items-center">
                            <svg class="flex-shrink-0 h-5 w-5 text-foreground-subtle" fill="currentColor" viewBox="0 0 20 20">
                                <path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd"/>
                            </svg>
                            <a href="{{ route('trips.listing') }}" class="ml-4 text-sm font-medium text-foreground-muted hover:text-foreground">My Trips</a>
                        </div>
                    </li>
                    <li>
                        <div class="flex items-center">
                            ...
                            <span class="ml-4 text-sm font-medium text-foreground">{{ $trip->destination }}</span>
                        </div>
                    </li>
                </ol>
            </nav>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-2 space-y-8">
                    ...
                    @if($weatherForecast)
                        <div class="grid grid-cols-1 md:grid-cols-7 gap-4">
                            @foreach($weatherForecast as $index => $weather)
                                <div wire:key="weather-{{ $index }}" class="text-center p-4 rounded-lg {{ $weather['date'] >= $trip->start_date->format('Y-m-d') && $weather['date'] <= $trip->end_date->format('Y-m-d') ? 'bg-primary/10 border border-primary/20' : 'bg-surface-muted dark:bg-surface' }}">
                                    <div class="text-sm font-medium text-foreground">{{ \Carbon\Carbon::parse($weather['date'])->format('M j') }}</div>
                                    @if($weather['icon'])
                ...
                <div class="card p-6">
                    <div class="flex items-center justify-between mb-4">
                        <h2 class="text-xl font-semibold text-foreground">Trip Notes</h2>
                        @if(!$editingNote)
                            <button wire:click="editNote" class="text-primary hover:text-primary-dark text-sm font-medium cursor-pointer">
                    ...
                    @if($editingNote)
                        <div class="space-y-4">
                            <textarea
                                ...
                                rows="6"
                                class="block w-full rounded-lg border-border bg-surface-card text-foreground focus:ring-primary focus:border-primary"
                                placeholder="Add your trip notes here..."
                            ></textarea>
                            <div class="flex space-x-2">
                                <button wire:click="saveNote" class="inline-flex items-center px-3 py-2 bg-primary text-primary-foreground rounded-lg hover:bg-primary-dark text-sm font-medium cursor-pointer">
                                    Save Notes
                                </button>
                                <button wire:click="cancelEditNote" class="inline-flex items-center px-3 py-2 border border-border text-foreground rounded-lg hover:bg-surface-muted text-sm font-medium cursor-pointer">
                                    Cancel
                                </button>
                            </div>
                        </div>
            ...
            <div class="space-y-8">
                <!-- Holidays -->
                <div class="card p-6">
                    <h2 class="text-xl font-semibold text-foreground mb-4">Holidays</h2>
                    @if($holidays)
                        <div class="space-y-3">
                            @foreach($holidays as $index => $holiday)
                                <div wire:key="holiday-{{ $index }}" class="flex items-start space-x-3 p-3 rounded-lg bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        ...
                                        <p class="text-xs text-foreground-muted">{{ \Carbon\Carbon::parse($holiday['date'])->format('M j, Y') }}</p>
                                        @if($holiday['description'])
                                            <p class="text-xs text-foreground-subtle mt-1">{{ $holiday['description'] }}</p>
                                        @endif
                                    </div>
                                </div>
                            @endforeach
                ...
                <!-- Expense Summary -->
                <div class="card p-6">
                    <h2 class="text-xl font-semibold text-foreground mb-4">Expense Summary</h2>
                            ...
                            @foreach($trip->expenses->groupBy('category') as $category => $expenses)
                                <div wire:key="expense-cat-{{ $category }}">
                                    <div class="flex items-center justify-between mb-2">
                                        <span class="text-sm font-medium text-foreground capitalize">{{ $category }}</span>
                                        <span class="text-sm text-foreground-muted">PKR {{ number_format($expenses->sum('amount')) }}</span>
                                    </div>
                                    <div class="space-y-1">
                                        @foreach($expenses as $expense)
                                            <div wire:key="expense-{{ $expense->id }}" class="flex items-center justify-between text-xs text-foreground-muted">
                        ...
                        @if($trip->updated_at != $trip->created_at)
                            <div class="flex items-start space-x-3">
                                ...
                                <div class="flex-1 min-w-0">
                                    ...
                                    <p class="text-xs text-foreground-muted">{{ $trip->updated_at->format('M j, Y \a\t g:i A') }}</p>
                                </div>
                            </div>
                        @endif
                        @if($trip->status === 'completed')
                            <div class="flex items-start space-x-3">
                                <div class="flex-shrink-0 w-2 h-2 bg-foreground-subtle rounded-full mt-2"></div>
                                <div class="flex-1 min-w-0">
                                    <p class="text-sm font-medium text-foreground">Trip Completed</p>
                                    <p class="text-xs text-foreground-muted">{{ $trip->updated_at->format('M j, Y \a\t g:i A') }}</p>
                                </div>
                            </div>
                        @endif
